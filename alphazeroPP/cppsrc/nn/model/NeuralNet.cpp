...
std::tuple< torch::Tensor, double > NetworkWrapper::predict(
...
std::vector< long > NetworkWrapper::prepend_to_shape(
   const torch::Tensor &tensor, size_t value) const
{
   // get current shape and initialize +1
   auto sizes = tensor.sizes();
   std::vector< long > sizes_out(
      static_cast< long >(sizes.size()) + 1);
