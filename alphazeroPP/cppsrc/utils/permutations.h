...
private:
    ...
    typename LastIter::value_type _indirection_elemwise(std::index_sequence<0>, LastIter &iter) const {
        return *iter;
    }
...
public:
    IteratorCollection(typename Containers::iterator &&... iters) : iterators(std::make_tuple(iters...)) {}
    ...
    bool operator==(const IteratorCollection &other_it) const {
        ...
        value_tuple own_values = _extract_iterator_values(
                iterators,
                idx_seq
        );
    ...
    template<size_t N>
    [[nodiscard]] auto &at() { return std::get<N>(iterators); }

    template<size_t N>
    [[nodiscard]] const auto &at() const { return std::get<N>(iterators); }
};
...
//    using reference_tuple = typename std::tuple<typename Containers::value_type &...>;
//    using pointer_tuple =  typename std::tuple<typename Containers::value_type *...>;
//    using iterator_tuple = typename std::tuple<typename Containers::iterator...>;
...
private:
//    typedef typename extract_iterator<Containers...>::type extract_iterator_tuple;
    template<size_t N>
