...
//   ~tqdm() { tqdm_register::pop_pbar(); }
   ...
   void progress(int curr, int tot)
   {
      if(is_tty && (curr % period == 0)) {
         ...
         double dt_tot = ((std::chrono::duration< double >) (now - t_first))
                            .count();
         int dn = curr - n_old;
         n_old = curr;
         t_old = now;
         ...
         // learn an appropriate period length to avoid spamming
         // stdout and slowing down the loop, shoot for ~25Hz and
         // smooth over 3 seconds
         if(nupdates > 10) {
            period = (int) (std::min(
               std::max((1.0 / 25) * curr / dt_tot, 1.0), 5e5));
            smoothing = 25 * 3;
         }
