...
class Board {
  public:
   ...
   [[nodiscard]] inverse_iterator begin_inverse(int team)
   {
      return m_map_inverse.at(team).begin();
   }

   [[nodiscard]] inverse_iterator end_inverse(int team)
   {
   ...
   std::array< inverse_map_type, 2 > const &get_inverse_map() const
   {
      return m_map_inverse;
   }

   const_inverse_iterator get_position_of_role(
      int team, const role_type &role) const
   {
...
template < typename PieceType >
std::tuple< bool, size_t > Board< PieceType >::check_bounds(
   const position_type &pos) const
{
   for(size_t i = 0; i < m_dim; ++i) {
      if(pos[i] < m_starts[i]
         || (pos[i] > 0 && (size_t) pos[i] >= m_shape[i])) {
...
template < typename PieceType >
std::shared_ptr< PieceType > &Board< PieceType >::operator[](
...
void Board< PieceType >::_fill_board_null_pieces(
   size_t dim, std::array< int, m_dim > &&position_pres)
{
   if(dim > 0) {
      for(int i = m_starts[dim - 1];
         ...
         _fill_board_null_pieces(
            dim - 1, std::forward< std::array< int, m_dim > >(position_pres));
      }
   } else {
...
{
   auto setup_unwrap =
      [&](const std::vector< std::shared_ptr< piece_type > > &setup) {
...
{
   auto setup_unwrap = [&](
                          const std::map< position_type, role_type > &setup,
                          int team) {
      ...
      for(auto &elem : setup) {
         ...
         if(std::find(seen_pos.begin(), seen_pos.end(), pos)
            != seen_pos.end()) {
            // element found
            throw std::invalid_argument(
               std::string("Parameter setup has more than one piece for the ")
               + std::string("same position (position: '") + pos.to_string()
               + std::string("')."));
         }
         seen_pos.emplace_back(pos);
         if(std::find(seen_role.begin(), seen_role.end(), character)
            != seen_role.end()) {
            throw std::invalid_argument(
               std::string("Parameter setup has more than one piece for the ")
...
void Board< PieceType >::_fill_inverse_board()
{
   for(const auto &piece_ptr : m_map) {
      ...
      if(! piece->is_null()) {
         m_map_inverse[piece->get_team()]
                      [piece->get_role()] = piece->get_position();
      }
   }
}
...
inline void Board< PieceType >::is_within_bounds(const position_type &pos)
{
   if(auto [in_bounds, idx] = check_bounds(pos); ! in_bounds) {
      ...
      std::string bounds_str = std::to_string(m_starts[idx]) + ", "
                               + std::to_string(m_shape[idx]);
