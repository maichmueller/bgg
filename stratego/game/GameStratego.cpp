   ...
   int team)
{
   ...
   while(! poss_pos.empty()) {
      ...
      setup_out[pos] = std::make_shared< piece_type >(
         pos, typename piece_type::token_type(type, --counter[type]), team);
...
int GameStratego::check_terminal()
{
   ...
   if(auto dead_pieces = state.get_graveyard(0);
      std::find(dead_pieces.begin(), dead_pieces.end(), token_type{0, 0}) != dead_pieces.end()) {
      ...
      state.set_terminality(WIN_1_FLAG);
             ...
             std::find(dead_pieces.begin(), dead_pieces.end(), token_type{0, 0})
             != dead_pieces.end()) {
   ...
   // Rule 2: If the moves of both teams have been repeated too often times.
      ...
      if(repetition_lambda(0) && repetition_lambda(1))
         // teams simply repeated their last 3 moves -> draw
         state.set_terminality(TIE);
   }
