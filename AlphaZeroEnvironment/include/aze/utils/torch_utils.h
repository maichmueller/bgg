...
struct NestedVectorManip< Vec< Val_Type, Alloc > > {
   ...
   template < typename size_type >
   static void sizes(
      const Vec< Val_Type, Alloc >& v, std::vector< size_type >& size)
   {
...
template <
   ...
   typename Val_Type,
   typename Alloc >
static void fill_tensor(
   const Container< Val_Type, Alloc >& vec, torch::Tensor& tensor_to_fill)
{
...
template <
   ...
   typename InnerAlloc >
static void fill_tensor(
   const OuterContainer< InnerContainer< InnerDType, InnerAlloc >, OuterAlloc >&
      vec,
   torch::Tensor& tensor_to_fill)
{
   for(int i = 0; i < vec.size(); ++i) {
      ...
      static_assert(
         std::is_same<
            decltype(next_level_vec),
            std::vector< InnerDType, InnerAlloc > >::value,
...
torch::Tensor tensor_from_vector(const std::vector< DType >& vec)
{
   ...
   // pass m_shape vector to ArrayRef (which is holding only the pointer to the
   // vector) for torch API
   ...
   //        auto tensor_access = tensor_to_fill.accessor<int64_t,
   //        NestedVectorManip<std::vector<DType>>::dimension>();
...
torch::Tensor tensor_from_vector(
   const std::vector< DType >& vec, torch::TensorOptions& options)
{
   /// Copies the vector (even nested vectors in vectors) data
   /// into a tensor and returns this tensor by copy.
