...
TEST_F(StateTest, StateTest_Constructor_Ints_2D)
{
   ...
   EXPECT_EQ(*(state[{4, 0}]), (piece_type({4, 0}, token_type{3, 0}, 1)));
   EXPECT_EQ(*(state[{1, 1}]), (piece_type({1, 1}, token_type{10, 0}, 0)));
   EXPECT_EQ(*(state[{0, 0}]), (piece_type({0, 0}, token_type{0, 0}, 0)));
   EXPECT_EQ(*(state[{3, 2}]), (piece_type({3, 2}, token_type{11, 0}, 1)));

   // copy state via board test
   auto state_copy = state_type(state.get_board(), 0);
   EXPECT_EQ(*(state_copy[{4, 0}]), (piece_type({4, 0}, token_type{3, 0}, 1)));
   EXPECT_EQ(*(state_copy[{1, 1}]), (piece_type({1, 1}, token_type{10, 0}, 0)));
   EXPECT_EQ(*(state_copy[{0, 0}]), (piece_type({0, 0}, token_type{0, 0}, 0)));
   EXPECT_EQ(*(state_copy[{3, 2}]), (piece_type({3, 2}, token_type{11, 0}, 1)));
