...
TEST_F(StateStrategoTest, StateStrategoTest_do_move) {
    ...
    ASSERT_EQ(state.get_board()->get_position_of_kin(1, {11, 0})->second, position_type(3, 2));
    ASSERT_EQ(state.get_board()->get_position_of_kin(0, {2, 2}), state.get_board()->end_inverse(0));

    move_type move2{{3, 1}, {2, 1}};
    state.do_move(move2);
    ASSERT_EQ((state[{3,1}]->get_kin()), kin_type(404, 404));
    ASSERT_EQ((state[{2,1}]->get_kin()), kin_type(2, 1));

    move_type move3{{1, 1}, {2, 1}};
    state.do_move(move3);
    ASSERT_EQ((state[{1,1}]->get_kin()), kin_type(404, 404));
    ASSERT_EQ((state[{2,1}]->get_kin()), kin_type(10, 0));
